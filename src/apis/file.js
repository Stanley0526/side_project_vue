...
import { storage } from "../firebase";  // 引入初始化的 Firebase storage
...
export async function uploadFile(file) {
  try {
    ...
    const storageRef = ref(storage, `files/${user.uid}/${file.name}`);  // 生成檔案的儲存參考位置，使用使用者 ID 作為資料夾名稱
